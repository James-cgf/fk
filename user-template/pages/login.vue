  ...
  <div class="main">
    ...
    <div class="sign-up-container">
      <el-form ref="userForm" :model="user">
        ...
        <div class="btn">
          <input type="button" class="sign-in-button" value="登录111" @click="submitLogin()">
        </div>
      </el-form>
      <!-- 更多登录方式 -->
      <div class="more-sign">
  ...
  methods: {
    ...
    submitLogin() {
      login.submitLogin(this.user).then(res => {
        ...
        // window.localStorage.setItem("fkStore",res.data.data.token)
        //第一个参数cookie名称，第二个参数值，第三个参数作用范围
        cookie.set("fkStoreToken", res.data.data.token, {domain: 'localhost'})
        //第四步 调用接口 根据token获取用户信息，为了首页面显示
        login.getLoginInfo().then(response => {
          this.userinfo = response.data.data.userInfo;
          //获取返回用户信息，放到cookie里
          cookie.set("fkUcenter", JSON.stringify(response.data.data.userInfo), {domain: 'localhost'})
          //跳转路径
